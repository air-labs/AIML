...
$$}
\uncover<14->{$$
\dd{E_k}{w_{21}}=\uncover<15->{\dd{E_k}{y_1}\dd{y_1}{w_{21}}+\dd{E_k}{y_2}\dd{y_2}{w_{21}}}\uncover<16->{=2(y_1-a_1)f'(S_1)x_2}
$$}
\end{columns}
\end{frame}
...
\end{array}
$$
\only<5>{Если бы $D_k=D_k(x_1,\ldots,x_m)$, то $\dd{E_k}{v_rs}=\sum_{j=1}^{m}\dd{D_k}{x_j}\dd{x_j}{w_{rs}}$}
\only<6-7>{$$
\begin{array}{c c}
\dd{E_k}{w_rs}=\sum_{j=1}^{m}\dd{D_k}{x_j}\dd{x_j}{w_{rs}} & 
