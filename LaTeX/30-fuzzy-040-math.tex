...
\draw[#1, ultra thick] (9,1-0.2*#3)--(9.5,1-0.2*#3);
\node[#1] at (10,1-0.2*#3) {$#4$};
}
...
\draw[red,thick] plot file {Plots/2.dat};
\only<+>{
...
\only<1-2>{
\draw[red,ultra thick] plot file{Plots/2.txt};
\draw[dotted] (2,1)--(2,0) node[below] {2};
\draw[dotted] (2,1)--(0,1) node[left] {1};
}
\only<2->{
\plotandlegend{orange}{4}{0}{4}
